...
bool GameTextFile::Read_CSF_Label(FileRef &file, StringInfo &string_info, int32_t &texts)
{
    ...
    if (rts::Read_Any(file, header)) {
        ...
        if (header.id == FourCC_LE<'L', 'B', 'L', ' '>::value) {
            if (rts::Read_Str(file, string_info.label, header.length)) {
                texts = header.texts;
                return true;
            }
        }
...
bool GameTextFile::Read_CSF_Text(FileRef &file, StringInfo &string_info)
{
...

    {
        ...
        if (rts::Read_Any(file, header)) {
            ...
            if (read_speech || read_text) {
                if (rts::Read_Str(file, string_info.text, header.length)) {
    ...
    if (read_speech) {
        ...
        if (rts::Read_Any(file, header)) {
            ...
            if (rts::Read_Str(file, string_info.speech, header.length)) {
                speech_ok = true;
            }
        }
    }
...
{
    bool ok = true;
    ok = ok && rts::Write_Str(file, string_info.label);
    ok = ok && rts::Write_Any(file, s_eol);
    return ok;
}
...
bool GameTextFile::Write_STR_Text(FileRef &file, const StringInfo &string_info, Option options)
{
    ...
    bool ok = true;
    ok = ok && rts::Write_Any(file, s_quo);
    ok = ok && rts::Write_Str(file, text_utf8);
    ok = ok && rts::Write_Any(file, s_quo);
    ok = ok && rts::Write_Any(file, s_eol);
    return ok;
}
...
bool GameTextFile::Write_STR_Speech(FileRef &file, const StringInfo &string_info)
{
    ...
    if (!string_info.speech.Is_Empty()) {
        ok = ok && rts::Write_Str(file, string_info.speech);
        ok = ok && rts::Write_Any(file, s_eol);
    }
    return ok;
}
...
{
    bool ok = true;
    ok = ok && rts::Write_Any(file, s_end);
    ok = ok && rts::Write_Any(file, s_eol);
    ok = ok && rts::Write_Any(file, s_eol);
    return ok;
}
...
bool GameTextFile::Write_CSF_Header(FileRef &file, const StringInfos &string_infos, const LanguageID &language)
{
    ...
    return rts::Write_Any(file, header);
}
...
bool GameTextFile::Write_CSF_Label(FileRef &file, const StringInfo &string_info)
{
    ...
    if (rts::Write_Any(file, header)) {
        if (rts::Write_Str(file, string_info.label)) {
            return true;
        }
    }
    return false;
...
bool GameTextFile::Write_CSF_Text(FileRef &file, const StringInfo &string_info, Utf16Buf utf16buf)
{
    ...
    if (text_ok && write_speech) {
        ...
        if (rts::Write_Any(file, header)) {
            if (rts::Write_Str(file, string_info.speech)) {
                speech_ok = true;
            }
        }
    }
